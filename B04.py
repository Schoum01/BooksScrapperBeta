...
# Fonction pour extraire les informations d'un livre et télécharger l'image
def scrape_book_data(product_page_url, image_dir):
    ...
    if image_response.status_code == 200:
        # Créer un nom de fichier basé sur le titre du livre (remplacer les espaces et caractères spéciaux)
        image_name = f"{title.replace(' ', '_').replace('/', '_').replace(':', '').replace('?', '').replace('#', '').replace('(', '').replace(')', '').replace('"', '').replace("'", '')}.jpg"
        image_path = os.path.join(image_dir, image_name)
        with open(image_path, 'wb') as img_file:
            shutil.copyfileobj(image_response.raw, img_file)
        print(f"Image téléchargée et enregistrée : {image_path}")
    else:
        print(f"Erreur: Impossible de télécharger l'image pour {title}. Code d'état: {image_response.status_code}")
...
def scrape_category(category_name, category_url):
    ...
    all_data = []
    image_dir = os.path.abspath(f'e:/2024_Python/BooksOnline/BOBenv/BOBenv/images/{category_name.replace(" ", "_").lower()}')
    os.makedirs(image_dir, exist_ok=True)  # Créer le dossier pour les images de la catégorie
    
    for url in product_urls:
        book_data = scrape_book_data(url, image_dir)
        if book_data:
